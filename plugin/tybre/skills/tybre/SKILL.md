...
- `tybre open <path> [--new-window] [--reuse-window] [--editor]` — Open a file in an editor tab of the running Tybre.md app, or a folder as its project (launches the app if it is closed)
...
- **`tybre agent`** — Manage AI teammates: list, hire, fire, and talk to them
  ...
  - `tybre agent react [--room] [--to] [--key] [--as] [--project]` — React to a message with a badge instead of a reply — use this for a simple acknowledgement ("check") so the room isn't filled with "OK". Reacting twice with the same key removes it
    - `--room`: Room id the message is in
    - `--to`: Message id to react to
    - `--key`: One of: eyes | check | thumbs_up | heart | laugh | question
    - `--as`: Optional check on your own name. Identity comes from the token this process was launched with, so naming another teammate is refused
